...
export function SessionSidebar() {
  ...
  return (
    <aside className="flex w-60 shrink-0 flex-col border-r border-gray-800 bg-[#13151e]">
      ...
      <nav className="flex flex-1 flex-col gap-0.5 overflow-y-auto p-2">
        {sessions.length === 0 && (
          <p className="px-2 py-3 text-center text-xs text-gray-500">No chats yet.</p>
        )}
          ...
          return (
            <div
              ...
              className={`group flex items-center gap-2 rounded-lg px-2 py-2 cursor-pointer transition-colors
                ${isActive ? 'bg-violet-900/50 text-violet-300' : 'text-gray-400 hover:bg-gray-800'}`}
