...
class VectorStoreSettings(BaseSettings):
    ...
    def ensure_directory_exists(cls, v: str | Path) -> Path:
        directory = Path(v)
        if not directory.is_file():  # Let DirectoryPath handle filepaths.
...
class LLMSettings(BaseSettings):
    ...
    max_output_token: int = Field(ge=1, description="Maximum tokens of LLM output.")
